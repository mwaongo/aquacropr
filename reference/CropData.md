...
Reference dataset containing all crop parameters for AquaCrop v7.0
...
A data frame with 83 rows and 5 columns:

- name:

  Parameter name (var_02 to var_84)
...
- var_05: Establishment method (0 = regrowth, 1 = sown, 2 =
  transplanted)
...
- var_11: Soil water depletion for canopy expansion (p_exp upper
  threshold)

- var_12: Soil water depletion for canopy expansion (p_exp lower
  threshold)
...
- var_66: Coefficient for positive impact of restricted vegetative
  growth (%)
...
- var_69-var_74: GDD equivalents of calendar day parameters (-9 if not
  used)
...
- var_81: Transfer of assimilates from above ground parts (0 = no, 1 =
  yes)
...
for writing crop files using these parameters
...
#>   <chr>  <dbl> <chr>                                                <chr> <dbl>
#> 1 var_02     7 AquaCrop Version (August 2022)                       %.1f     14
#> 2 var_03     1 File not protected                                   %.0f     15
#> 3 var_04     2 fruit/grain producing crop                           %.0f     15
#> 4 var_05     1 Crop is sown                                         %.0f     15
#> 5 var_06     1 Determination of crop cycle   by calendar days       %.0f     15
#> 6 var_07     1 Soil water depletion factors (p) are adjusted by ETo %.0f     15
...
#>   name   value description                                           fmt   width
#>   <chr>  <dbl> <chr>                                                 <chr> <dbl>
#> 1 var_35  1.05 Crop coefficient when canopy is complete but prior t… %.2f     15
CropData[CropData$name == "var_50", ] # Maximum canopy cover
#> # A tibble: 1 × 5
#>   name   value description                                       fmt   width
#>   <chr>  <dbl> <chr>                                             <chr> <dbl>
#> 1 var_50   0.8 Maximum canopy cover (CCx) in fraction soil cover %.2f     15
...
#>   name   value description                                           fmt   width
#>   <chr>  <dbl> <chr>                                                 <chr> <dbl>
#> 1 var_53    90 Calendar Days  from sowing to maximum rooting depth   %.0f     15
#> 2 var_54    65 Calendar Days  from sowing to start senescence        %.0f     15
#> 3 var_55    90 Calendar Days  from sowing to maturity (length of cr… %.0f     15
#> 4 var_56    52 Calendar Days  from sowing to flowering               %.0f     15
#> 5 var_57    20 Length of the flowering stage (days)                  %.0f     15
#> 6 var_58     1 Crop determinancy linked with flowering               %.0f     15
```
